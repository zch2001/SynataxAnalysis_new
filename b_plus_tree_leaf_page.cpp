...
int B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key,
    ...
    // empty or larger than the max one, insert in the last pos
    if (GetSize() == 0 || comparator(key, KeyAt(GetSize() - 1)) > 0)
    {
        array[GetSize()] = { key, value };
    }
    // less than the min one, insert in the first one
    else if (comparator(key, array[0].first) < 0){
        memmove((void*)(array + 1), array, static_cast<size_t>(GetSize() * sizeof(MappingType)));
        array[0] = { key, value };
    }
    //���ֲ��Ҷ�Ӧ����λ��
    else{
...
bool B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType &value,
    ...
    //if empty or less than minimum or larger than maximum, find nothing
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::CopyFirstFrom(
    ...
    auto parent = reinterpret_cast<BPlusTreeInternalPage<KeyType, 
        decltype(GetPageId()),KeyComparator>*>(page->GetData());
